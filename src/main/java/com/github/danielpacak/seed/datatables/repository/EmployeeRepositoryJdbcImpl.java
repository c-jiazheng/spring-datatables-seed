...
public class EmployeeRepositoryJdbcImpl implements EmployeeRepository {
  ...
  @Override
  public List<Employee> findAll(DataTablesRequest dataTablesRequest) {
    return jdbcTemplate.query("SELECT ID, FIRST_NAME, LAST_NAME, POSITION, OFFICE, AGE, START_DATE, SALARY FROM EMPLOYEE LIMIT ? OFFSET ?",
      new Object[]{dataTablesRequest.getLength(), dataTablesRequest.getStart()}, new RowMapper<Employee>() {
        @Override
  ...
  @Override
  public long countAll(DataTablesRequest dataTablesRequest) {
